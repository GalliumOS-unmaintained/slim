...
bool Panel::CheckXRandr() {
	int i;
	int major, minor;
	int error;
	
	/* Check for XRandr and ask to be notified on changes */
	if (!XRRQueryExtension (Dpy, &randr_event, &error)) {
		fprintf(stderr, "Error querying xrandr\n");
		return False;
	}
	...
	if (major != 1 ||  minor < 2) {
		fprintf(stderr, "Unsupported xrandr version\n");
		return False;
	}
...
#  else  /* !RRScreenChangeNotifyMask */          /* Xrandr.h 1.4, 2001/06/07 */
		XRRScreenChangeSelectInput (Dpy, RootWindow (Dpy, i), True);
...
void Panel::WrongPassword(int timeout) {
	...
	if (mode == Mode_Lock) {
		...
		width = XWidthOfScreen(ScreenOfDisplay(Dpy, Scr));
		height = XHeightOfScreen(ScreenOfDisplay(Dpy, Scr));
	}
	
	int msg_x = Cfg::absolutepos(cfgX, width, extents.width);
	int msg_y = Cfg::absolutepos(cfgY, height, extents.height);
		...
		if (XPending(Dpy) || poll(&x11_pfd, 1, -1) > 0) {
			while(XPending(Dpy)) {
			  ...
			  if (event.type == randr_event + RRScreenChangeNotify) {
			    ...
			    new_viewport = GetPrimaryViewport();
